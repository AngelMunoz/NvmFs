...
let installCommand = command "install" {
  ...
  inputs(
    ...
    Input.OptionMaybe<bool>(
      [ "-l"; "--lts" ],
      "Ignores version and pulls down the latest LTS version"
    ),
    Input.OptionMaybe<bool>(
      [ "-c"; "--current" ],
      "Ignores version and pulls down the latest Current version"
    ),
    Input.Option<bool>(
      [ "-d"; "--default" ],
      false,
      "Sets the downloaded version as default"
    )
  )
...
let listCommand = command "list" {
  ...
  inputs(
    Input.OptionMaybe<bool>(
      [ "-r"; "--remote" ],
      "Displays the last downloaded version index in the console"
    ),
    Input.OptionMaybe<bool>(
      [ "-u"; "--update" ],
...
let rootHandler(ctx: InvocationContext) = task {
  let hc =
    HelpContext(
      ctx.HelpBuilder,
