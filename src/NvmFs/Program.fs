...
let installCommand = command "install" {
  ...
  inputs(
    ...
    optionMaybe "--lts" |> alias "-l" |> desc "Ignores version and pulls down the latest LTS version",
    optionMaybe "--current" |> alias "-c" |> desc "Ignores version and pulls down the latest Current version",
    option "--default" |> alias "-d" |> def false |> desc "Sets the downloaded version as default"
  )
...
let listCommand = command "list" {
  ...
  inputs(
    optionMaybe "--remote" |> alias "-r" |> desc "Displays the last downloaded version index in the console",
    optionMaybe "--update" |> alias "-u" |> desc "Use together with --remote, pulls the version index from the node website"
  )
