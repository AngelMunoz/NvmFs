...
module IO =
  ...
  let createSymlink (actualPath: string) (symbolicLink: string) =
    try
      ...
      Debug.WriteLine(
        $"Created symlink {dirInfo.FullName} -> {dirInfo.LinkTarget}"
      )
    ...
    with ex ->
      ...
      Error
        $"Could not write symlink {symbolicLink} -> {actualPath}\n {ex.Message}"
  ...
  let removeSymlink(path: string) =
    ...
    | :? IOException as ex ->
      Error $"Failed to delete symlink: {path} - [red]{ex.Message}[/]"
  ...
  let getCurrentNodeVersion(os: CurrentOS) = task {
    let cmd =
      match os with
  ...
  let versionDirectory (os: CurrentOS) (version: string) =
    Path.Combine(
      Common.getHome(),
      version,
      (if os = Windows then String.Empty else "bin")
    )
  ...
  let versionExistsInDisk (codename: string) (version: string) =
    ...
    let exists =
      directories |> Array.exists(fun dir -> dir.Name.Contains(codename))
