...
module Network =
    ...
    let downloadWithPwsh url fileDestination =
        task {
            let work =
                Cli
                    .Wrap("pwsh")
    ...
    let downloadWithCurl url fileDestination =
        task {
            let work =
                Cli
                    .Wrap("curl")
    ...
    let downloadWithWGet url fileDestination =
        task {
            let work =
                Cli
                    .Wrap("wget")
    ...
    let downloadNode (version: string) (os: CurrentOS) (arch: string) =
        ...
        let downloadFile (url: string) path =
            ...
            |> TaskResult.orElseWith (fun error ->
                ...
                |> TaskResult.map (fun _ -> File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read)))
